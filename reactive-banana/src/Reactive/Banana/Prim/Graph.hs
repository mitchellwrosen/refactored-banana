...
emptyGraph :: Graph a
emptyGraph = Graph HashMap.empty HashMap.empty HashSet.empty

-- | Insert an edge from the first node to the second node into the graph.
insertEdge :: (Eq a, Hashable a) => (a, a) -> Graph a -> Graph a
insertEdge (x, y) gr =
  gr
    { children = HashMap.insertWith (flip (++)) x [y] (children gr),
      parents = HashMap.insertWith (flip (++)) y [x] (parents gr),
      nodes = HashSet.insert x $ HashSet.insert y $ nodes gr
    }

-- | Get all immediate children of a node in a graph.
getChildren :: (Eq a, Hashable a) => Graph a -> a -> [a]
getChildren gr x = fromMaybe [] . HashMap.lookup x . children $ gr

-- | Get all immediate parents of a node in a graph.
getParents :: (Eq a, Hashable a) => Graph a -> a -> [a]
getParents gr x = maybe [] id . HashMap.lookup x . parents $ gr
...
listParents gr = list
  where
    ...
    ancestors :: [a]
    ancestors = [x | x <- HashSet.toList $ nodes gr, null (getParents gr x)]
...
dfs' xs0 succs = fst <$> go xs0 [] HashSet.empty
  where
    go :: [a] -> [a] -> HashSet a -> m ([a], HashSet a)
    go [] ys seen = pure (ys, seen) -- all nodes seen
    go (x : xs) ys seen
      | x `HashSet.member` seen = go xs ys seen
      | otherwise = do
        xs' <- succs x
        -- visit all children
        (ys', seen') <- go xs' ys (HashSet.insert x seen)
